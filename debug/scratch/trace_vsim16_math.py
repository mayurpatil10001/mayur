...
def trace_vsim16_dec18_pos_math():
    ...
    print(f"--- V_SIM16 Dec 18 Position Math Tracing ---")
    pos = 0
    for f in fills:
        ...
        ny_dt = f['dt'] - datetime.timedelta(hours=5) if f['dt'] else None
        dt_str = ny_dt.strftime('%H:%M:%S') if ny_dt else "?"
        
        # Log if we hit > 6 or if it's a note-less fill
        if abs(pos) > 6 or not f['has_note']:
            status = "NOTELESS" if not f['has_note'] else "OVERLIMIT"
            print(f"[{status}] {dt_str} | Side: {f['side']} | Qty: {f['qty']} | POS: {old_pos} -> {pos} | Msg: {f['msg'][:80]}")

if __name__ == "__main__":
    trace_vsim16_dec18_pos_math()
