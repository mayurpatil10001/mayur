...
p("Tracing Position from 00:00 NY to 05:00 NY")
p(f"{'Time':<12} | {'Side':<4} | {'Qty':<3} | {'Px':<8} | {'Note':<25} | {'Msg':<40} | {'Tag126':<5} | {'Pos'}")
p("-" * 120)
...
for f in fills:
    ...
    if ny.hour < 5:
        ...
        if ny.hour >= 1:
            p(f"{is_ghost} {ny.strftime('%H:%M:%S')} | {side:<4} | {qty:<3} | {px:<8.2f} | {note[:25]:<25} | {msg:<40} | {t126:<5} | {pos}")

with open("diag_pos.txt", "w", encoding="utf-8", newline="\n") as f:
    f.write("\n".join(out))
