...
p(f"Time         | S | Qty | Px       | Note                      | Msg")
p('-' * 110)

for f in fills:
    ny = f['ny']
    # Check if time is between 02:18 and 04:09 on Dec 18
    ...
    if ny.date().day == 18:
        if (ny.hour == 2 and ny.minute >= 18) or ny.hour == 3 or (ny.hour == 4 and ny.minute <= 9):
            ...
            p(f"{ny.strftime('%H:%M:%S')} | {side:1} | {qty:3} | {px:8.2f} | {note:25} | {msg:40}")

with open('diag_chart_window.txt', 'w', encoding='utf-8') as fh:
    fh.write('\n'.join(out))
